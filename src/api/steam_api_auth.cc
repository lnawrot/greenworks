...
NAN_METHOD(CancelAuthTicket) {
  ...
  HAuthTicket h = info[1].As<v8::Number>()->Int32Value(Nan::GetCurrentContext()).FromJust();
...
NAN_METHOD(GetEncryptedAppTicket) {
  ...
  char* user_data = *(static_cast<Nan::Utf8String>(info[0]->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>())));
  if (!user_data) {
    THROW_BAD_ARGS("Bad arguments");
  }
  Nan::Callback* success_callback =
...
NAN_METHOD(IsTicketForApp) {
  ...
  uint32 app_id = info[1]->Uint32Value(Nan::GetCurrentContext()).FromJust();
