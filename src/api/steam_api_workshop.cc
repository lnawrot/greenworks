...
NAN_METHOD(PublishWorkshopFile) {
  ...
  Nan::AsyncQueueWorker(new greenworks::PublishWorkshopFileWorker(
      success_callback, error_callback, app_id.ToLocalChecked()->Int32Value(Nan::GetCurrentContext()).FromJust(),
      properties));
  info.GetReturnValue().Set(Nan::Undefined());
}
...
NAN_METHOD(UGCGetItems) {
  ...
  auto ugc_query_type = static_cast<EUGCQuery>(info[2]->Int32Value(Nan::GetCurrentContext()).FromJust());
  ...
  Nan::AsyncQueueWorker(new greenworks::QueryAllUGCWorker(
      ...
      app_id.ToLocalChecked()->Int32Value(Nan::GetCurrentContext()).FromJust(),
      page_num.ToLocalChecked()->Int32Value(Nan::GetCurrentContext()).FromJust()));
  info.GetReturnValue().Set(Nan::Undefined());
}
...
NAN_METHOD(UGCGetUserItems) {
  ...
  auto ugc_list = static_cast<EUserUGCList>(info[3]->Int32Value(Nan::GetCurrentContext()).FromJust());
  ...
  Nan::AsyncQueueWorker(new greenworks::QueryUserUGCWorker(
      ...
      ugc_list_order, app_id.ToLocalChecked()->Int32Value(Nan::GetCurrentContext()).FromJust(),
      page_num.ToLocalChecked()->Int32Value(Nan::GetCurrentContext()).FromJust()));
  info.GetReturnValue().Set(Nan::Undefined());
}
...
NAN_METHOD(UGCGetDetails) {
  ...
  PublishedFileId_t handle = utils::strToUint64(
      *(Nan::Utf8String(info[0])));
  Nan::Callback* success_callback =
      new Nan::Callback(info[1].As<v8::Function>());
  Nan::Callback* error_callback = NULL;
  ...
  if (info.Length() > 3 && info[3]->IsInt32())
    page_num = info[3]->Int32Value(Nan::GetCurrentContext()).FromJust();
...
void RegisterAPIs(v8::Local<v8::Object> target) {
  ...
  SET_FUNCTION("ugcGetDetails", UGCGetDetails);
}
